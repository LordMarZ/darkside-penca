...
// PostgREST corta selects en 1000 filas por defecto — con cientos de
// usuarios prediction supera ese limite, así que hay que paginar o el
// recalculo ignora en silencio todas las filas mas allá de la primera página.
...
export async function recomputeAllPoints(supabase) {
  const results = await selectAll(supabase, 'match_results')
  const predictions = await selectAll(supabase, 'predictions')
  ...
  const profiles = await selectAll(supabase, 'profiles', 'id')
  ...
  for (const result of sortedResults) {
    ...
    for (const pred of preds) {
      ...
      // Postgres valida las columnas NOT NULL del candidate row de un
      // upsert ANTES de resolver el conflicto — aunque la fila ya exista
      // y solo vayamos a tocar pts_earned, hay que mandar también las
      // columnas obligatorias (con su valor actual, sin cambiarlas).
      predUpdates.push({
        id: pred.id,
